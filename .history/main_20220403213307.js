window.addEventListener("load", function(){
    ...
    // click bên listAlbumMusic thì phát nhát và đổi tên bên phát nhạc
    ...
    function changeMusic(x,list){
        if(x===1){ // next
                if(local == true) {
                    // index là vị trị hiện tại trong ListItemMusic
                    index = localStorage.getItem("data");
                    local = false;
                }
            index++;
        ...
        else if(x===-1) //prev
        {
            if(local == true) {
                // index là vị trị hiện tại trong ListItemMusic
                index = localStorage.getItem("data");
                // chỉ lưu 1 lần , rest lại khi click vào ListItemMusic
                local = false;
            }
    ...
    function clickAlbumAuthorItem(e){
        ...
        [...listAlbumAuthorItem].forEach(function(item) {
             ...
             item.classList.remove("active");
             e.currentTarget.classList.add("active");
             // mỗi lần chuyển album rest lại vị trí đầu tiên
             localStorage.setItem("data",0);      
             local = true;        
         // mỗi lần chuyển album active vị trí đầu tiên         
        [...listItemMusic].forEach(function(item)
             {
                 item.classList.remove("active");
                 if(item.getAttribute("data-music") ==0)
                 {
                     item.classList.add("active");
                 }
             })
     // click vào AlbumAuthor  gắn giá trị cho listAuthor vào local : để phân biệt đang phát nhạc list nào
        if(e.currentTarget.dataset.item == 0)
             {  
                 localStorage.removeItem("listAuthor");
                 localStorage.setItem("listAuthor",0)
                 // chạy bản nhạc đầu tiên trong list
                 song.setAttribute("src",`./audio/${list1[0].link}`)
                 playerName.textContent = list1[0].name;
                 playerAuthor.textContent =  list1[0].author;
        ...
        else if(e.currentTarget.dataset.item ==1)
             {
                ...
                song.setAttribute("src",`./audio/${list2[0].link}`)
                playerName.textContent = list2[0].name;
                playerAuthor.textContent =  list2[0].author;
        ...
        else if(e.currentTarget.dataset.item ==2)
             {
                ...
                song.setAttribute("src",`./audio/${list3[0].link}`)
                playerName.textContent = list3[0].name;
                playerAuthor.textContent =  list3[0].author;
             }
        else if(e.currentTarget.dataset.item ==3)
             {
                localStorage.removeItem("listAuthor");
                localStorage.setItem("listAuthor",3)
                song.setAttribute("src",`./audio/${list4[0].link}`)
                playerName.textContent = list4[0].name;
                playerAuthor.textContent =  list4[0].author;
             }
        else if(e.currentTarget.dataset.item ==4)
             {
                localStorage.removeItem("listAuthor");
                localStorage.setItem("listAuthor",4)
                song.setAttribute("src",`./audio/${list5[0].link}`)
                playerName.textContent = list5[0].name;
                playerAuthor.textContent =  list5[0].author;
             } 
        });
